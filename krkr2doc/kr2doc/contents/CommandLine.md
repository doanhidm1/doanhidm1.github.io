...
吉里吉里のコマンドラインオプションは通常のコマンドラインから指定するほかに、[Releaser](Releaser) ( krkrrel.exe ) または [吉里吉里設定](Configure) ( krkrconf.exe または エンジン設定.exe ) で設定ファイルに保存することができます。
オプションが読み込まれる順序は
...
　環境依存の問題の解決については[環境依存の不具合について](ComputerDependentProblems)も参照してください。
...
下のリストの中で「動的に変更可能」という表記がある物は、[System.setArgument](f_System_setArgument) メソッドで変更が可能な物です。それ以外のオプションは動的に変更を行うことは出来ません。
...
-debug (デバッグモード)
:   吉里吉里をデバッグモード  ( → [デバッグ](Debug) )  で動作させるかどうかの設定です。
...
以下のオプションは、吉里吉里の[ホットキー](HotKeys)関連のオプションです。
　設定できる値は[MenuItem.shortcut](f_MenuItem_shortcut) プロパティで指定できる文字列と同様です。
...
:   [Layer.holdAlpha](f_Layer_holdAlpha) プロパティのデフォルト値を指定します。
