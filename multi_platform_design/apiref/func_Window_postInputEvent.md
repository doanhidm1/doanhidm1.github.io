...
:   |  |  |
    | --- | --- |
    | eventname | イベント名称を指定します。  以下の文字列で指定します。   * "onKeyDown" は Window.onKeyDown イベントを生成します。 * "onKeyPress" は Window.onKeyPress イベントを生成します。 * "onKeyUp" は Window.onKeyUp イベントを生成します。   onKeyDownとonKeyUpは対になるので、onKeyDownを生成したら対応するonKeyUpも生成することを推奨します。 |
