...
switch ステートメントを使うと、if 縲鰀 else if を簡潔に書くことができます。以下の構文をとります。
...
　base\_expression には式を書き、最初にこの式が評価されます。switch の次のブロックの中の case の次の condition\_expression? にも式を書きます。
　condition\_expression? はブロック内で次々に評価されます。評価された結果が base\_expression と同じ場合、そこから実行が開始されます。condition\_expression? に合致しない場合は、その間にかかれた文やステートメントは無視されます。また、合致した場合、それ以降、case や default は無視されます。
