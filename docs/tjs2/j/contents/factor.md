...
項は、式評価の中でもっとも最高の優先順位にあるものです。これらは、通常の即値、識別子  ( → [ãã¼ã¯ã³](token) )  の他、以下のものがあります。
...
`function` を使って、式中に関数を書くことができます。[関数](function) を参照してください。

# 式中配列

`[ ]` を使って、式中に `Array` クラスのオブジェクトをその場で記述することができます。[Array クラス](array) を参照してください。

# 式中辞書配列

`%[ ]` を使って、式中に `Dictionary` クラスのオブジェクトをその場で記述することができます。[Dictionary クラス](dictionary) を参照してください。