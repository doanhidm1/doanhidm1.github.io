...
　[Storages.getFullPath](f_Storages_getFullPath) メソッドは、OS の使うファイル名から統一ストレージ名への変換を行うことができます。この過程で、ファイル名中の大文字はすべて小文字に変換されます ( ローカルストレージについては大文字と小文字は区別されません )。また、パス区切りに \ が用いられている場合は / に変換されます。
　[Storages.getLocalName](f_Storages_getLocalName) メソッドは、統一ストレージ名から OS の使うファイル名への変換を行うことができます。吉里吉里以外のアプリケーションにストレージ名を渡す場合などはこのメソッドを使用する必要があります。
...
　吉里吉里は標準で XP3 という吉里吉里独自形式のアーカイブストレージを扱うことができます ( XPK の第３世代の形式なので XP3 )。XP3 形式のアーカイブの作成方法については [Releaser](Releaser) を参照してください。
...
データ保存場所は [コマンドラインオプション](CommandLine) の -datapath オプションで指定されたフォルダです。[System.dataPath](f_System_dataPath) プロパティで取得することができます。
