...
:   |  |  |
    | --- | --- |
    | meta | 読み込んだ画像のタグ情報です。 |
    | async | 非同期で読み込まれたかどうかです。  　画像データがキャッシュ内にある場合、 [Bitmap.loadAsync](f_Bitmap_loadAsync) 実行中に本イベントが発生します。 |
    | error | 画像読込みでエラーが発生したかどうかです。 |
    | message | エラーメッセージです。  　エラーが発生した場合、エラーメッセージが渡されます。 |

説明
:   非同期読込みが完了した時に発生します。
    　読み込んだ画像は [Layer.copyFromBitmapToMainImage](f_Layer_copyFromBitmapToMainImage) で Layer にコピーできます。
...
参照
:   [Bitmap.loadAsync](f_Bitmap_loadAsync)
