...
吉里吉里のコマンドラインオプションは通常のコマンドラインから指定するほかに、[Releaser](Releaser) ( krkrrel.exe ) または [吉里吉里設定](Configure) ( -userconf ) で設定ファイルに保存することができます。
オプションが読み込まれる順序は
...
:   設定可能な値は *'Shift\_JIS' (吉里吉里2互換の動作)*, *'UTF-8' (吉里吉里Zの動作)* のいずれかで、このオプションを指定しないと 'UTF-8' が指定されたものと見なされます。
